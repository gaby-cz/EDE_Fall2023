...
#Option 1
df <- cbind(df_car_speed,car_dist)  #similarly rows could be added using rbind() if you want to append records to the end of your data frame
class(df)
df
...
```{r}
#useful function data.frame() -- use data.frame command when you want to create a data frame from scratch and as.data.frame when you have something you want to convert to data frame
...
summary(df) #generates summary statistics for your data frame. it'll report min, quartiles, mean, max
head(df) #show the first 6 rows of df

#If you know the number of the column you want you can refer to that to access column
df[,1] #when you forget to put a row, it returns everything in the first column if you don't specify a value, but otherwise do [row,column] and can also run slices which is the start:end of the rows or columns. you can also give it a vector of row values in additional parentheses if you want specific rows. if you add a negative number for a row or column, it will remove that row or column
