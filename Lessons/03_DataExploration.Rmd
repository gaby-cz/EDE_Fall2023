...
# Working directory should be set to the base folder for the Environmental Data Analytics Course repository, i.e., the folder that houses the R Project file. R prints the absolute path of your working directory.
...
Create the best practice of having a chunk just to load/install your packages rather than doing it in the middle of the script.At the top of your R scripts, you should load any packages that need to be used for that R script. A common issue that arises is that packages will be loaded in the middle of the code, making it difficult to run specific chunks of code without scrolling to make sure all necessary packages are loaded. For example, the tidyverse package is one that we will use regularly in class.
...
> Answer: the output will show on your console. for tidyverse it's showing all packages that come with tidyverse
...
* *file = * : this is the file name. if you're using the practice of one folder for your lessons, another for your assignments, etc. you will have to specify not only the file name but also the location. use this input to point to your data file. If it's on the same folder as your .Rmd then you only need to write the file name. But if it's on another folder you need to point to the path were file is located;
* *header =* : if your file has a header you should set this to TRUE, o.w. FALSE; header = TRUE is usually the default, so it will have name of column in first row and first entry of data on the second row
...
* *dec =* : define *dec="."* or *dec=","* depending on how it's defined on your set. The default is ".". This is important esp if you're working with international data.

```{r Importing data}
# Absolute file path (not recommended because while /home/guest/ etc might be on Luana's computer, they are not on my computer)
#read.csv("/home/guest/EDE_Fall2023/Data/Raw/USGS_Site02085000_Flow_Raw.csv",stringsAsFactors = TRUE)

# Relative file path (friendly for users regardless of machine; it's relative to working directory so as long as we have the same project, our path related to the wd will be the same--data under data folder, raw under raw folder, etc.) Putting in the dot means it's gonna show you all the options for files relative to your wd
USGS.flow.data <- read.csv("./Data/Raw/USGS_Site02085000_Flow_Raw.csv",stringsAsFactors = TRUE)   

# What happens if we don't assign a name to our imported dataset? The data isn't being stored! You want to choose a name for the object where u want your data to be stored, here (above) we can use USGS
#read.csv("./Data/Raw/USGS_Site02085000_Flow_Raw.csv",stringsAsFactors = TRUE)

# Another option is to choose with your browser -- a finder or explorer window will pop up and you can point to the data file you want to import. 
# read.csv(file.choose(),stringsAsFactors = TRUE)

# To import .txt files, use read.table rather than read.csv -- CSV will only work for comma-separated file, so if you want to read a text file, you need to use the read.table function
#read.table()
...
class(USGS.flow.data) #checking class to make sure data was stored in df
colnames(USGS.flow.data) #checking column names. it's assigning first row as names for columns but these names are not informative so we want to rename those columns.

# Rename columns. instead of just calling colnames, we will assign a vector to it, which should have the same # of elements as # of columns in the df. so we're creating a vector w the name of each column, binding all names together into a vector (got the info from the readme file)
...
str(USGS.flow.data) # this shows ur df and all objects u hve in ur df
dim(USGS.flow.data) # will tell u how many rows and columns u have in dataset
length(USGS.flow.data) # tells how many columns u have

head(USGS.flow.data) # shows first few rows of ur data (6 first unless u specify how many)
head(USGS.flow.data, 10)
tail(USGS.flow.data, 5) #shows last few / bottom of data set
USGS.flow.data[30000:30005, c(3, 8, 14)] # can use matrix notation to access chunks of the frame, row 30k to 30k5, but just columns 3,8,14

class(USGS.flow.data$datetime) #figure out type of each column (pointing to the column ur interested in)
class(USGS.flow.data$discharge.mean)
class(USGS.flow.data$gage.height.mean)

summary(USGS.flow.data)  #could point to column only with $ ; gives u summary statistics which makes sense for columns like discharge max
summary(USGS.flow.data$discharge.max)

```

What happened to blank cells in the spreadsheet when they were imported into R?
> Answer: data was not available, R imported it as NA. if you run summ statistics for the column it'll show the NAs

## Adjusting Datasets

### Removing NAs

Notice in our dataset that our discharge and gage height observations have many NAs, meaning no measurement was recorded for a specific day. In some cases, it might be in our best interest to remove NAs from a dataset. Removing NAs or not will depend on your research question. If the missing observations are in the beginning of historic data, you can just remove them, but when they're in the middle, need to be careful how we remove them. For timeseries analyses, we never remove NAs but we do use other methods to replace NAs with another value. For some types of analysis, we can just omit the NAs and work with the data we actually have.
...
```{r Removing NAs}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #takes one main argument, which is the data frame from which we want to remove our NAs. Create a new object to store the NA-removed data without the NAs instead of removing from original
dim(USGS.flow.data) #shows u 33k observations on original data.
dim(USGS.flow.data.complete) # shows u new data set with NAs removed (haven't removed any columns, just the rows that had NAs)

mean(USGS.flow.data.complete$discharge.mean) #now can compute mean
