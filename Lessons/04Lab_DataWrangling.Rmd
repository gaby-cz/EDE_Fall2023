...
#Alternatively, use the tidyverse/dplyr "glimpse" function - the tidyverse analogy to the str function 
...
Filtering allows us to choose certain rows (observations) in our dataset. When we filter rows, we have to tell r which rows we want to keep. If we look at lakeID (a factor) and depth is numeric...
...
# dplyr filtering
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0) # grab all records where depth is 0

# Choose multiple conditions to filter -- now we have our categorical variable, or our factor, and this command reports all the 
summary(NTL.phys.data$lakename) #could also use unique() or levels() with the same arguments
NTL.phys.data.PeterPaul <- 
  filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #saying if the value falls in this list it's a keeper

# Choose a range of conditions of a numeric or integer variable
summary(NTL.phys.data$daynum)
NTL.phys.data.JunethruOctober <- filter(NTL.phys.data, daynum %in% c(152:304)) #using the in operator but now we're providing a slice
```
...
NTL.phys.data.TuesLake90thru99 <- filter(
  NTL.phys.data, 
  lakename == "Tuesday Lake" & year4 %in% c(1990:1999) #the equivalent of %in% is not inclusive of the endpoints in python
  )
...
Pipe is another method to wrangle datasets that looks cleaner and is easier to read.  We designate a pipe with `%>%`. A good way to think about the function of a pipe is with the word "then." The keyboard shortcut for %>% is shift+command+m.
...
```{r Exercise 3 & 4}
#Example using pipes to wrangle data -- atomic commands we're running one after another after another using the pipes
NTL.phys.data.processed <- 
  NTL.phys.data %>% #this command is piping something into what's next -- it'll be the first value in the command
