...
#Explore
class(GasPlants) # it's an sf (simple features) object which means we can do spatial analysis with it
names(GasPlants) # there's a geometry column
str(GasPlants$geometry) # looking at the structure of the geometry column we see it's a simple feature column point

#View using sf plot
plot(GasPlants['status'],pch=16) # the sf package has a plot function. we can see status (Active or Inactive) 

#View using ggplot
ggplot(data=GasPlants) + geom_sf() # the points here are in geographic space so we see longitude and latitude, points kind of follow the shape of NC.

#View using mapview
mapview(GasPlants) # mapview reads spatial dataframes and puts them with a nice backdrop. it also pulls in the attributes. can play around with diff backdrops. can also export this as a webpage.

```


## EXERCISE: Creating a spatial dataframe from coordinate columns. How do we take data that has location info but not as a geometry column and how do we convert that into a spatial data frame?
...
# we have latitude and longitude values and we can use those to create the geometry. we'll simplify things by collapsing or distilling the data so it only has one record for each site on mean ozone and mean PM2.5
...
EPAair.sites <- EPAair %>% 
  ...
  summarize(
    ...
    count_PM25 = sum(!is.na(PM2.5))) # went from 9k records to 13. we still have the site info longitude and latitudes so that's what we want
# na.rm = TRUE -- you want to do it this way instead of doing just the n() because...

#Convert to a spatial dataframe - want to take the long and lat columns and use st_as_sf to create the simple table as a simple feature
EPAair.sf <- EPAair.sites %>% 
  # tell the code what columns to use as x and y coordinates. longitude is actually x, latitude is y. !!! they're inverted!
  st_as_sf(coords = c('mean_Lng','mean_Lat'),
           crs=4326) #specifying the coordinate reference system

#View the data using the methods above... notice that you lost the long and lat because they got collapsed in the geometry column. now we have the spatial dataframe which we can see by class() -- has "sf" which means we can do spatial analysis with it! 

```

### EXERCISE: Creating a spatial dataframe from known coordinates. -- if you know the coordinates and want to create an sf from those spatial coordinates. Simple feature geometry is just the location. Simple feature column is the location plus the coordinate reference system and then the data frame.

```{r Create a spatial dataframe from scratch}
#First create a simple feature geometry object ("sfg")
sfg_GraingerHall <- st_point(c(-78.9427,36.00465)) # the class is xy. it's a simple feature point (the sf geometry / the most elemental thing) 
class(sfg_GraingerHall)
mapview(sfg_GraingerHall) # it doesn't know where on the planet to put a coordinate. doesn't know how the coordinate links to the planet.

#Upgrade to a simple feature geometry column by adding a crs 
sfc_GraingerHall <- st_sfc(sfg_GraingerHall,crs=4326) # take the simple geom and link it with a crs. the crs 4326 essentially says these xy refer to lat long. the st_sfc says it's a simple features column.
class(sfc_GraingerHall) # it's a point not a line or polygon
mapview(sfc_GraingerHall) # will now put it in the correct location bc it's associated w a crs tying the coord to a location on earth

#Upgrade to a simple feature table or a spatial dataframe using st_as_sf. it knows it's a geometry so we don't have to specify the columns
sf_GraingerHall <- st_as_sf(sfc_GraingerHall)
sf_GraingerHall$Name = 'Grainger Hall'
mapview(sf_GraingerHall)
# now we can associate it with attributes using our standard methods. but, if we wanted to calculate a distance, how would we say that in degrees?
...
Each of the above datasets we explored use lat/long coordinates in defining their geometries. Lat/long are spherical or angular coordinates. That process is called projection. We actually specify that these use lat/long coordinate when creating them via the `crs = 4326` parameter you see in the examples. But what is "4326"?? And what is the significance of this? 
...
the_buffer <- sf_GraingerHall %>% st_buffer(1) # buffering is simple spatial analysis where u take a geometry and expand it outward a certain unit of measure. map the buffer and see that if we buffer grainger hall one unit, we're buffering it in spherical coordinates so it won't look like a perfect circle on a plane so we need to reconfigure our data sets so they map out well on a plane. there will be some sort of distortion, and that distortion can affect our data. taking data that's on a sphere and mapping it onto a plane or flat surface.
