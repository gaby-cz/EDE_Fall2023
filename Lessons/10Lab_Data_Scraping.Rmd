...
#Set the URL
# saving the web address that gives us what we want, as a string
theURL <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02087500&period=7&format=rdb'

#Get the data, which starts on line 30
# instead of reading in a file locally, we're having it read in from the cloud
# can pull into text editor and see what number line the data starts at
...
gage_data <- gage_data %>% 
  ...
  # convert datetime to an actual date and time
...
# the discharge is really all the same. the "P" means it was published but not confirmed. The "A" would mean approved/authorized/good to go. the provisional data should mean you really take it with a grain of salt.
...
# saving the web address that gives us what we want, as a string
# you're banking on the response of the web service to be uniform every time
...
Application programming interfaces, or APIs, are a formalization of the REST (when you hear a REST-based API, know you can probably work with it in R) request/response process. More precisely, they are web sites that have been designed to allow a client to query specific bits of data from an on-line resource without just "hacking" a URL.
...
#rvest is the following three commands
...
# the catch: any time you scrape data it'll be pulled in as text, so you need to use as.numeric. However, as.numeric will give you an error bc it has a comma in there. You find the gsub command which substitutes a substring. look at the lab slides.
the_state_df <- cross2(the_states,the_price) %>% 
  map(lift(scrape.it)) %>% 
  bind_rows()
...
# the map2 function takes two inputs
# if you don't include the last bit it says here's a list of dfs, one for each year you put in
