...
# pulls the webpage into our coding environment. it's a set of nodes and a set of documents
...
# putting these as diff variables makes them easier to go back and change
the_base_url <- 'https://www.ncwater.org/WUDC/app/WWATR/report/view'
the_facility <- '0004-0001'
the_year <- 2020
# paste0 eliminates adding a space between the elements we want to concatenate
...
scrape.it <- function(the_year, the_facility){
  ...
  #Pause for a moment - scraping etiquette. This just tells R to wait for 1 second. But you could tell it to wait for 10 seconds. If we're iterating for 2015, 16, 17, etc., we might want to communicate to the server that it's not a denial of server attack--so you don't want to violate the robots text and get your access cut off. Or cut off Duke's access... So, add a little 5 second pause as part of the etiquette. 
