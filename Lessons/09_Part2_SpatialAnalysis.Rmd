...
counties_sf<- st_read(here('Data/Spatial/cb_2018_us_county_20m.shp')) %>% 
  filter(STATEFP == 37) #Filter for just NC Counties. can apply tidy tools because it's a st_ data frame. 
...
neuse_sf<- st_read(here('Data/Spatial/NCHUC8.shp')) %>% 
  filter(SUBBASIN == 'Upper Neuse') #Filter for just NC Counties. can apply tidy tools because it's a st_ data frame. 
...
ggplot() +
  ...
  geom_sf(data = neuse_sf, fill = 'orange', size=2) 

#or

mapview(huc8_sf, col.regions='purple', layer.name = 'HUC 8') +
  mapview(filtered_HUCs, col.regions = 'orange', 
          layer.name = 'Upper Neuse', alpha=1)

```

### 2.2.4 Reading in GeoJSON data

More and more spatial data are appearing on-line, in a format that allows us to connect directly to the data vs downloading local copies to our workspace. An example is the Homeland Infrastructure Foundation-Level Data (HIFLD) Their open data site (<https://hifld-geoplatform.opendata.arcgis.com/>) has links to many datasets.
...
wind_raw <- read.csv(
  'https://raw.githubusercontent.com/ENV859/
  EnviroAtlasData/main/Wind_Energy.csv',
  # get it to just be a factor with the colClasses command
  colClasses = c('HUC_12' = 'factor')) %>%  
  # this is making an HUC 8 from HUC 12
...
#View the data
mapview(counties_sf)
# now every feature has the same value (in GIS, this is called a dissolve bc we're dissolving all the boundaries btwn two features that have the same value)
mapview(state_sf)
```
...
# we'll aggregate the 8-digit HUCs into 6-digit HUCs
# the first five subbasins are all within the Roanoke Basin. we want to basically compress them into one feature and spatially dissolve the boundary. Then we'll tabulate the sum of the acres and the sum of the sq miles.
unique(huc8_sf$DWQ_Basin) # we should have 17 features bc there are 17 unique values

#Summarize on DWQ Basin value
huc2_sf <- huc8_sf %>%
  group_by(DWQ_Basin) %>%
  # we'll update the area by summing the acres in the original acres)
...
#Select intersecting counties using matrix subsetting. just grabbing the counties that intersect neuse_sf (4th row, 3rd column. but if u want everything in the 4th row u just don't specify the column)
...
# there's also st_within and st_contains and st_is_within_distance
#e.g.
the_filter <- st_is_within_distance(
  x = counties_sf_utm,
  y=neuse_sf,
  dist = 100000,
  sparse = FALSE
...
# you could also filter by saying:
neuse_intersect_2 <- counties_sf_utm %>% st_filter(neuse_sf)
                            
# generates a series with 100 entries. there are 100 counties in NC
