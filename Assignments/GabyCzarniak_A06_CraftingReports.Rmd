...
```{r project.setup, message=FALSE, warning=FALSE, echo=TRUE}
#Import libraries
library(tidyverse);library(lubridate);library(here);library(knitr)

# Importing EPA data (from the Processed_KEY folder) & fixing dates
epa_data <- read.csv(
  here("Data",
       "Processed_KEY",
...
summary_PM25 <- epa_data %>% 
  ...
  # ordering the rows of the table
  arrange(mean_PM2.5)
...
# using 'kable' function to produce table of Mean PM2.5 by county
knitr::kable(summary_PM25, 
             caption = "Mean Particulates (2.5mm)", 
             col.names = c("County", "$\\mu g/m^3$")
             )
# using 'kable' function to produce table of Mean Ozone by county             
...
```{r Ozone boxplot 2018, fig.align='left', fig.width=5.25, fig.height=3.5, fig.cap= "Ozone Levels in 2018", messsage=FALSE, warning=FALSE, echo=FALSE}

# creating boxplot for just 2018
ozone_2018_boxplot <- 
  ggplot(subset(epa_data, Year == "2018"), 
         # changing months to show as month names, not numbers
         aes(x=factor(Month, levels = 1:12, 
                      labels = month.abb), 
             y=Ozone)) +
    geom_boxplot() +
    ggtitle("Recorded Ozone by Month in 2018") +
    # changing labels and formatting units
    labs(x = "Month") + 
    ylab(expression(paste("Ozone Concentration (",mu,")"/m^3)))
print(ozone_2018_boxplot)
```


```{r Ozone boxplot 2019, fig.align='left', fig.width=5.25, fig.height=3.5, fig.cap="Ozone Levels in 2019", message=FALSE, warning=FALSE, echo=FALSE}
...
ozone_2019_boxplot <- 
  ggplot(subset(epa_data, Year == "2019"), 
        ...
        y=Ozone)) +
    geom_boxplot() +
    ggtitle("Recorded Ozone by Month in 2019") +
    # changing labels and formatting units
    labs(x="Month") + 
    ylab(expression(paste("Ozone Concentration (",mu,")"/m^3)))
...
## Git Repository
https://github.com/gaby-cz/EDE_Fall2023
